...
// StringEqualFold 比较UTF-8编码在小写的条件下是否相等，不区分大小写
func StringEqualFold(t Fataler, actual, expected string, a ...interface{}) {
	if !strings.EqualFold(actual, expected) {
		fatal(cond{
			Fataler:    t,
			Format:     "expected these to be equal:\nACTUAL:\n%s\nEXPECTED:\n%s",
			FormatArgs: []interface{}{actual, expected},
			Extra:      a,
		})
	}
}

// StringNotEqualFold 比较UTF-8编码在小写的条件下是否不相等，不区分大小写
func StringNotEqualFold(t Fataler, actual, expected string, a ...interface{}) {
	if strings.EqualFold(actual, expected) {
		fatal(cond{
			Fataler:    t,
			Format:     "expected two different values, but got the same:\n%s",
			FormatArgs: []interface{}{actual},
			Extra:      a,
		})
	}
}
